...
class SwaggerRelationshipGenerator extends BaseGenerator
{
    ...
    public function __construct($relationship, $model, $modelCurrent)
    {
        ...
        $this->_generate($relationship, $model, $modelCurrent);
    }

    private function _generate($relationship, $model, $modelCurrent): void
    {
        $fileName = "$model.php";
        ...
        if ($relationship === $this->relationship['has_one'] || $relationship === $this->relationship['has_many']) {
            ...
            $templateRequired =
                self::REQUIRED .
                $this->serviceGenerator->searchTemplate(
                    self::REQUIRED,
                    '}',
            ...
            $templateData = $this->serviceGenerator->replaceNotDelete(
                ...
                $this->_generateField($modelCurrent, $relationship),
                1,
                $templateData,
            );
            $templateData = $this->serviceGenerator->replaceNotDelete(
                $this->notDelete['json_content'],
                $this->_generateField($modelCurrent, $relationship, true),
                0,
                $templateData,
            );
        } else {
            ...
            $templateData = $this->serviceGenerator->replaceNotDelete(
                $this->notDelete['property'],
                $this->_generateField($model, $relationship),
                1,
                $templateData,
            );
            $templateData = $this->serviceGenerator->replaceNotDelete(
    ...
    private function _generateField($modelRelationship, $relationship, $isPropertyJson = false): string
    {
        if ($isPropertyJson) {
